...
fun CodeEditor(
    ...
    val lineHeight = RlDimens.codeLineHeight
    val lineHeightPx = with(density) { lineHeight.toPx() }
    val verticalScroll = rememberScrollState()
    val horizontalScroll = rememberScrollState()
    val measurer = rememberTextMeasurer()

    // Monospace advance width, measured once so the gutter and scroll extent line up.
    val charWidth = remember(measurer) {
        val measured = measurer.measure("0000000000", RlType.code.copy(fontSize = 12.5.sp))
    ...
    BoxWithConstraints(modifier.fillMaxSize().background(RlColors.Panel)) {
        ...
        val gutterWidth = RlDimens.gutterWidth
        ...
        Box(
            ...
            Row(Modifier.size(totalWidth, totalHeight)) {
                ...
                Box(
                    ...
                    Box(
                        Modifier
                            .offset(y = RlDimens.codePaddingTop + lineHeight * (activeLine - 1))
                            .fillMaxWidth()
                            .height(lineHeight)
                            .background(RlColors.PaperDeep.copy(alpha = 0.55f)),
                    )
                    BasicTextField(
                        value = value,
                        onValueChange = { next ->
                                ...
                                when {
                                    event.key == Key.Tab -> {
                                        val insertion = if (event.isShiftPressed) {
                                            ...
                                            insertAtCursor(value, " ".repeat(TAB_WIDTH))
                                        }
                                        value = insertion
                                        onTextChange(value.text)
                                        true
...
private fun insertNewlineWithIndent(value: TextFieldValue): TextFieldValue {
    ...
    val extra = if (currentLine.trimEnd().endsWith("{") || currentLine.trimEnd().endsWith("(")) " ".repeat(TAB_WIDTH) else ""
    val insertion = "\n$indent$extra"
...
private fun removeIndent(value: TextFieldValue): TextFieldValue {
    ...
    var removed = 0
    while (removed < TAB_WIDTH && caret > 0 && text[caret - 1] == ' ') {
        caret--
        removed++
    }
