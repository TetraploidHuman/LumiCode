...
- **行高亮带对齐**（踩过坑）：Compose 在显式 `lineHeight` 大于字体自身行高时，是把字形**贴行框顶部**
  排的（实测基线 = 行框顶 + ascent），多余 leading 全留在下方 —— 于是「行阴影里的代码」看起来偏上，
  而 `LineHeightStyle.Alignment.Center` 并不生效（wasm 上还会因字体异步加载量到不同结果）。
  现在**不给代码设 `lineHeight`**：行框就等于字体自身行高，字形正好填满，阴影带天然包住字形；
  行距则用 `onTextLayout` 从**真实文本布局**里量（`getLineTop(1) - getLineTop(0)`），
  不再依赖字体度量，四端一致。
